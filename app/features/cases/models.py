...
from sqlalchemy import (
    Boolean, Date, ForeignKey, Integer, SmallInteger, String, Text,
...
class Case(Base):
    ...
    bench_type: Mapped[BenchType] = mapped_column(
        SAEnum(BenchType, name="bench_type"), nullable=False, default=BenchType.single_bench
    )
    stage: Mapped[CaseStage] = mapped_column(
        SAEnum(CaseStage, name="case_stage"), nullable=False, default=CaseStage.filing
    )
    status: Mapped[CaseStatus] = mapped_column(
        SAEnum(CaseStatus, name="case_status"), nullable=False, default=CaseStatus.active, index=True
    )
    ...
    next_hearing_date: Mapped[date | None] = mapped_column(Date, index=True)
    ...
    context: Mapped[Optional["CaseContext"]] = relationship("CaseContext", back_populates="case", uselist=False)
    summary: Mapped[Optional["CaseSummary"]] = relationship("CaseSummary", back_populates="case", uselist=False)
    drafts: Mapped[list["Draft"]] = relationship("Draft", back_populates="case", cascade="all, delete-orphan")
    

    # Relationships
    lawyer: Mapped["User"] = relationship("User", back_populates="cases", foreign_keys=[lawyer_id])  # type: ignore[name-defined]
    case_numbers: Mapped[list["CaseNumber"]] = relationship(
        "CaseNumber", back_populates="case", cascade="all, delete-orphan"
    )
    case_sections: Mapped[list["CaseSection"]] = relationship(
        "CaseSection", back_populates="case", cascade="all, delete-orphan"
    )
    case_accesses: Mapped[list["CaseAccess"]] = relationship(
        "CaseAccess", back_populates="case", cascade="all, delete-orphan"
    )
    parties: Mapped[list["Party"]] = relationship(
        "Party", back_populates="case", cascade="all, delete-orphan"
    )
...
class CaseSection(Base):
    ...
    case: Mapped["Case"] = relationship("Case", back_populates="case_sections")
    added_by_user: Mapped["User"] = relationship("User", foreign_keys=[added_by])  # type: ignore[name-defined]
    removed_by_user: Mapped["User | None"] = relationship("User", foreign_keys=[removed_by])  # type: ignore[name-defined]
...
class CaseAccess(Base):
    ...
    case: Mapped["Case"] = relationship("Case", back_populates="case_accesses")
    user: Mapped["User"] = relationship("User", foreign_keys=[user_id])  # type: ignore[name-defined]
    granted_by_user: Mapped["User"] = relationship("User", foreign_keys=[granted_by])  # type: ignore[name-defined]
